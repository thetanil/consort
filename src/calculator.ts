...
import { isTypeParameterDeclaration, ScriptSnapshot, StringLiteralLike } from 'typescript';
...
// type SSMessageId = string;
type SSChannel = { channel: string };

interface SSMessage extends JSONObject { };
// type SendFn<T> = (msg: T) => Promise<SSMessage>

type InEnvelope = SSMessage & SSChannel

let myMsg: InEnvelope = { channel: 'mymessage', a: 1 }
console.log(myMsg)

namespace nodejs {
    export const testfn = (): string => { return '' }
    export const handlers = { 'testfn': testfn }
}
...
const add = async (e: Event, msg: { x: number, y: number }): Promise<number> => {
    return msg.x + msg.y
}
let services = {
    'add': add
